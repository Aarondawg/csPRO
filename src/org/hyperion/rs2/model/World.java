...
public class World {
	...
	public void unregister(NPC npc) {
		npcs.remove(npc);
		npc.destroy();
		npc.getActionQueue().cancelQueuedActions();
	}
	...
	public void register(final Player player) {
		...
		if (BannedUsers.contains(NameUtils.formatName(player.getName()))) {
			returnCode = 4;
		}
		if (!players.add(player) && returnCode == 2) {
			returnCode = 7;
			logger.info("Could not register player : " + player
					+ " [world full]");
		}
	...
	public void handleError(Throwable t) {
		...
		try {
			 ...
			 * Process p;
			 * 
			 * r = Runtime.getRuntime(); //System.out.println("Going to Exec");
			 * 
			 * //try {
			 * 
			 * p = load.exec("D:/Workspace/PJA Up To Date/run.bat");
			 * 
			 * //} catch (Exception e) {
			 */
			// System.out.println("Unexpected exception Trying to Execute Job: "
			// + e);
			// Runtime.getRuntime().exec("D:/Workspace/PJA Up To Date/run.bat");
		} catch (Exception e) {
			e.printStackTrace();
		}
		System.exit(1);
