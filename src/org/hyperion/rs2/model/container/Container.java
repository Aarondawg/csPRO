...
public class Container {
	...
	/**
	 * Checks if the container contains the specified item id and amount.
	 * 
	 * @param id
	 *            The item id.
	 * @param amount
	 ...
	 *            The item.
	 * @param offset
	 *            If we have to consider an extra amount of items added.
	 * @return <code>true</code> if so, <code>false</code> if not.
	 */
	public boolean hasRoomFor(Item item, int offset) {
		if (item.getDefinition().isStackable()
				|| type.equals(Type.ALWAYS_STACK)) {
			for (int i = 0; i < items.length; i++) {
				if (items[i] != null && items[i].getId() == item.getId()) {
			...
			int slots = freeSlots() + offset;
			return slots >= item.getCount();
		}
	}

	/**
	 * Checks if there is room in the inventory for a container of items.
	 * 
	 * @param item
	 *            The item.
	 * @param extraAmount
	 *            An extra amount of spaces to include in the check.
	 * @return <code>true</code> if so, <code>false</code> if not.
	 */
	public boolean hasRoomFor(Container container, int extraAmount) {
		Container con = new Container(type, capacity); // We create a copy of
														// this container.
		for (Item item : items) {
			if (item != null) {
				con.add(item); // And put all items from this container in the
								// copyed container.
			}
		}
		for (int i = 0; i < extraAmount; i++) {
			if (!con.add(new Item(-1))) { // If we can't add all the items,
				return false; // We return false.
			}
		}
		for (Item item : container.toArray()) {// We loop through the items in
												// the container we want to add
												// items from..
			if (item != null) {
				if (!con.add(item)) { // If we can't add all the items,
					return false; // We return false.
				}
			}
		}
		...
		for (int i = 0; i < items.length; i++) {
			if (items[i] != null) {
				if (items[i].getId() == item.getId()) {
					amtOf += items[i].getCount();
				}
			}
		}
		return amtOf >= item.getCount();
	}
