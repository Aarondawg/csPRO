...
public class DumbPathFinder implements PathFinder {
	...
	public Path findPath(Location location, int radius, TileMap map, int srcX,
		...
		int stepX = 0, stepY = 0;
		if (srcX > dstX
				&& map.getTile(dstX, srcY).isWesternTraversalPermitted()) {
			stepX = -1;
		} else if (srcX < dstX
				&& map.getTile(dstX, srcY).isEasternTraversalPermitted()) {
			stepX = 1;
		}
		if (srcX > dstY
				&& map.getTile(srcX, dstY).isSouthernTraversalPermitted()) {
			stepY = -1;
		} else if (srcX > dstY
				&& map.getTile(srcX, dstY).isNorthernTraversalPermitted()) {
			stepY = 1;
		}
		if (stepX != 0 || stepY != 0) {
			Path p = new Path();
