...
public class LoginConnectionHandler extends IoHandlerAdapter {
		...
		server.pushTask(new Runnable() {
			public void run() {
				if (session.containsAttribute("node")) {
					((Node) session.getAttribute("node"))
							.handlePacket((LoginPacket) message);
				} else {
					handlePreAuthenticationPacket(session,
							(LoginPacket) message);
				}
			}
		});
	...
	public void sessionOpened(IoSession session) throws Exception {
		session.getFilterChain().addFirst("protocolCodecFilter",
				new ProtocolCodecFilter(new LoginCodecFactory()));
	}
	 ...
	 * @param message
	 */
	private void handlePreAuthenticationPacket(IoSession session,
			LoginPacket message) {
