...
public class Node {
	...
	public void handlePacket(LoginPacket packet) {
		...
		case LoginPacket.CHECK_LOGIN: {
			String name = NameUtils.formatNameForProtocol(IoBufferUtils
					.getRS2String(buf));
			...
			if (res.getReturnCode() == 2) {
				PlayerData pd = new PlayerData(name, res.getPlayer()
						.getRights().toInteger());
		...
		case LoginPacket.LOAD: {
			String name = NameUtils.formatNameForProtocol(IoBufferUtils
					.getRS2String(buf));
			Player p = new Player(new PlayerDetails(null, name, "", 0, null,
					null));
